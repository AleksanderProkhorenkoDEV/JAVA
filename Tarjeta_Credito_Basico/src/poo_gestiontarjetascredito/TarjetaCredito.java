...
public class TarjetaCredito {
    ...
    public TarjetaCredito(TarjetaCredito tarjeta) {
        ...
        tarjeta.añoCaducidad = this.añoCaducidad;
        tarjeta.mesCaducidad = this.mesCaducidad;
    ...
    * Metodo getter, devuelve el mes de caducidad de la tarjeta
    ...
    public boolean pagar(double cantidad, String concepto) {
        ...
        if (Funciones.ValidarCadenaTexto(concepto, 100, 1)) {
            throw new IllegalArgumentException("El concepto introducido no cumple con el minimo de caracteres");
        }
        if ((cantidadGastada + cantidad) < cantidad) {
            aceptarPago = true;
    ...
    public static int obtenerDigitoControl(String numero) {
        ...
        int digito;
        int sumaDigitos = 0;
        if (!Funciones.validarNumerosDeLaTarjeta(numero)) {
            throw new IllegalArgumentException("El numero de la tarjeta de credito no cumple con los parametros establecidos");
        }
    ...
    public boolean comprobarNumeroTarjeta(String numero) {
        ...
        numCadena = numero.charAt(numero.length());
        int digitoControl = TarjetaCredito.obtenerDigitoControl(numero);
        if (digitoControl == numCadena) {
            esValido = true;
        }
        return esValido;
    }
